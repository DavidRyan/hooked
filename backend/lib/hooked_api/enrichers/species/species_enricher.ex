defmodule HookedApi.Enrichers.Species.SpeciesEnricher do
  ...
  def enrich(user_catch, context \\ %{}) do
    ...
    Logger.debug(
      "SpeciesEnricher: Context local_image_path: #{inspect(context[:local_image_path])}"
    )

    try do
      case @species_provider.validate_configuration() do
        :ok ->
          case identify_species_from_image(user_catch.image_url, context) do
        ...
        {:error, reason} ->
          Logger.error(
            "SpeciesEnricher: Species provider configuration error for catch #{user_catch.id}: #{inspect(reason)}"
          )
  ...
  defp identify_species_from_image(image_url, context) when is_binary(image_url) do
    ...
    try do
      with {:ok, file_path} <- get_image_path(image_url, context),
        ...
        @species_provider.identify_species(file_path, filename)
      else
  ...
  defp identify_species_from_image(image_url, _context) do
  ...
  defp get_image_path(image_url, %{local_image_path: path}) when is_binary(path) do
    if File.exists?(path) do
      Logger.debug("SpeciesEnricher: Using local image file: #{path}")
      {:ok, path}
    else
      Logger.debug("SpeciesEnricher: Local file not found, falling back to S3 download")
      ImageStorage.get_image_file_path(image_url)
    end
  end

  defp get_image_path(image_url, _context) do
    Logger.debug("SpeciesEnricher: No local path provided, downloading from S3")
    ImageStorage.get_image_file_path(image_url)
  end
